...
def uspsclass(tr_fea,tr_label,ts_fea,ts_label):
    ...
    for i in range(mtest):
        classifierresult = classify(ts_fea[i],tr_fea,tr_label,3)
    ...
    print("\nkNN's total number of accuracy is: %d" % accuracycount, file=data)
    print("\nkNN's total accuracy rate is: %f" % (accuracycount/float(mtest)), file=data)
    data.close()
